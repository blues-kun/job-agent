...
const fields = [["city","city"],["intent","intent"],["education","education"],["years","experience_years"],["salary","salary_min"],["salary-max","salary_max"],["district","district"]];
...
function renderResults(result) {
  ...
  $("results").innerHTML = result.jobs.map(job => {
...
for(const id of ["resume-text","city","intent","education","years","salary","salary-max","district","strict","query-mode","research-consent"]) $(id).addEventListener("input",()=>{if(id==="resume-text")clearSamplePreferences();const pair=fields.find(pair=>pair[0]===id);if(pair)state.origins[pair[1]]="user_input";$("sample").value="";invalidate();});
