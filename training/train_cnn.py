...
class JobRecommendDataset(Dataset):
    ...
    def __init__(self, X, y):
        # CNN 需要 3D 输入: (batch, channels, length)
        # 将特征reshape为 (1, num_features) 以模拟1个通道
        self.X = torch.FloatTensor(X).unsqueeze(1)  # (N, 1, features)
...
class CNNModel(nn.Module):
    """卷积神经网络模型"""
    def __init__(self, input_dim, num_filters=64, kernel_sizes=[3, 5, 7], dropout=0.3):
    ...
    def forward(self, x):
        ...
        for conv in self.convs:
            conv_out = conv(x)  # (batch, num_filters, features)
            conv_out = torch.max(conv_out, dim=2)[0]  # Global max pooling
...
def train_cnn(X_train, y_train, X_val, y_val, input_dim):
    """训练 CNN 模型"""
    print("\n[5/6] 训练 CNN 模型...")
    ...
    model = CNNModel(input_dim, num_filters=64, kernel_sizes=[3, 5, 7], dropout=0.3).to(device)
...
def main():
    ...
    model = train_cnn(X_train, y_train, X_val, y_val, X.shape[1])
    ...
    with open('models/cnn_meta.json', 'w', encoding='utf-8') as f:
